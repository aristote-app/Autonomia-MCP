...
export function normalizeTerritoryMarketSignal(item, territory, query) {
  const classification = classifyTerritoryMarketSignal(item);
  if (!classification || !territory?.id || !item?.buyerSiren) return null;
  ...
  return {
    ...
    payload: {
      ...
      buyer_siren: clean(item.buyerSiren),
