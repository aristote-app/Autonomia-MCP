...
import {
  ...
  WEB_DEMAND_QUERY_SPECS
} from "../collectors/webDemandDiscovery.js";
import { discoverFranceTravailDemand } from "../collectors/franceTravail.js";
import { persistTerritoryProgramSignals } from "../db/signals.js";
...
export async function runAutomatedJobSignalRefresh({
  ...
  try {
    ...
    return {
      ...
      fallbackReason,
      discoveredRows: jobItems.length + territoryProgramItems.length,
      persistedRows:
        (persistence.count || 0) +
        (territoryProgramPersistence.signalRows || 0),
      jobSignals: {
        discoveredRows: jobItems.length,
        persistedRows: persistence.count || 0
      },
      territoryPrograms: {
        discoveredRows: territoryProgramItems.length,
        persistedRows: territoryProgramPersistence.signalRows || 0,
        error: territoryProgramError
      },
...
export async function runAutomatedExtendedDemandRefresh({
  ...
  const selectedWebSpecs = includeWeb
    ? WEB_DEMAND_QUERY_SPECS.filter(
        (spec) => !Array.isArray(webKinds) || webKinds.length === 0 || webKinds.includes(spec.kind)
      )
    : [];
  ...
  const runId = await startCollectorRun({
    ...
    queryPayload: {
      ...
      webKinds: Array.isArray(webKinds) ? webKinds : null,
      sources: [
        ...new Set([
          ...selectedWebSpecs.map((spec) => spec.sourceId),
  ...
  try {
    const [webResult, franceTravailResult] = await Promise.allSettled([
      includeWeb
        ? discoverWebDemandSignals({ specs: selectedWebSpecs })
        : Promise.resolve({
            available: false,
    ...
    const territoryProgramItems = webItems.filter(
      (item) => item?.rawPayload?.search_kind === "territory_program"
    );
    const jobItems = [
      ...webItems.filter((item) => item?.rawPayload?.search_kind !== "territory_program"),
      ...(franceTravail.items || [])
    ];

    const persistence = await upsertJobSignals(jobItems);
    let territoryProgramPersistence = {
      persisted: true,
      sourceRecords: territoryProgramItems.length,
      signalRows: 0,
      results: []
    };
    let territoryProgramError = null;

    if (territoryProgramItems.length) {
      try {
        territoryProgramPersistence = await persistTerritoryProgramSignals(
          territoryProgramItems,
          { collectorRunId: runId }
        );
      } catch (error) {
        territoryProgramError =
    ...
    await finishCollectorRun(runId, {
      status: territoryProgramError ? "partial" : "success",
      stats: {
        ...
        territory_programs_discovered: result.territoryPrograms.discoveredRows,
        territory_programs_persisted: result.territoryPrograms.persistedRows,
        territory_programs_error: result.territoryPrograms.error,
