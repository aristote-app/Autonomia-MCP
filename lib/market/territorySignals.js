...
const TERRITORY_PUBLIC_COLLECTORS = Object.freeze({
  ...
  marches_securises: ({ query, limit }) =>
    searchMarchesSecurises({ query, page: 1, limit })
});

export const DEFAULT_TERRITORY_PUBLIC_SOURCES = Object.freeze([
  "boamp",
  "ted",
  "marches_securises"
]);

function normalizeSourceList(sources = DEFAULT_TERRITORY_PUBLIC_SOURCES) {
  ...
  return supported.length ? supported : [...DEFAULT_TERRITORY_PUBLIC_SOURCES];
}
...
export async function runAutomatedTerritorySignalRefresh({
  ...
  sources = DEFAULT_TERRITORY_PUBLIC_SOURCES,
  limitPerQuery = 30,
