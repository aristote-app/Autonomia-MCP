...
function extractReference(text) {
  return clean(
    String(text || "").match(/Référence\s*(?:interne\s*)?\|?\s*([^|]+?)(?=Type de marché|Procédure|Zone\(s\)|Date de publication|$)/i)?.[1]
  );
}

function splitListings(text) {
  const markers = [...String(text || "").matchAll(/Référence\s*\|/gi)];
...
function parseChunk(chunk) {
  ...
  const beforeRef = text.split(/Référence\s*\|/i)[0] || "";
