...
const SEARCH_SPECS = Object.freeze([
  ...
  { kind: "territory", query: "intelligence artificielle collectivité" },
  { kind: "territory", query: "chef de projet IA collectivité" },
  { kind: "territory", query: "intelligence artificielle territorial" },
  { kind: "territory", query: "automatisation IA collectivité" }
]);
...
function textOf(offer) {
  return [
    ...
    ...(offer?.origineOffre?.partenaires || []).map((partner) => partner?.nom)
  ]
    .filter(Boolean)
    .join(" ");
}
...
function isTerritoryEvidence(text) {
  return /collectivit[eé]|territorial|territoriaux|mairie|municipal|communaut[eé] de communes|communaut[eé] d['’]agglom[eé]ration|m[eé]tropole|d[eé]partement|r[eé]gion|\bEPCI\b|syndicat mixte|\bPETR\b|centre de gestion|service public/i.test(
    text
  );
}
...
function territorySignalKeys(text) {
  ...
  if (isTrainingEvidence(text)) keys.push("training_need", "territory_ai_training");
...
export function normalizeFranceTravailOffer(offer, spec) {
  ...
  const signalKeys = [
    ...
    ...(territory ? territorySignalKeys(text) : [])
  ];
  ...
  return {
    ...
    contractType: territory
      ? "Signal recrutement collectivité IA"
      : training
        ? "Besoin formation IA"
        : freelance
  ...
  for (const spec of specs) {
    try {
      ...
      const normalized = offers
        .map((offer) => normalizeFranceTravailOffer(offer, spec))
        .filter(Boolean);
