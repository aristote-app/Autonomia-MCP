...
export function assertSuccessfulWaalaxyImport(payload, { campaignId = null } = {}) {
  const item =
    payload?.result?.[0] ||
    payload?.data?.[0] ||
    payload?.prospects?.[0] ||
  ...
  if (!item) {
    throw new Error("Waalaxy import returned no per-prospect result");
  }

  const importCode = String(item.importCode || "");
  const importOk = [
    "success",
  ...
  if (!importOk) {
    throw new Error(
      "Waalaxy import did not succeed: " +
      (importCode || item.message || "unknown_import_result")
    );
  }

  if (campaignId) {
    const campaignCode = String(item.addToCampaignCode || "");
    if (!["success", "already_in_campaign"].includes(campaignCode)) {
      throw new Error(
        "Waalaxy campaign enrolment did not succeed: " +
        (campaignCode || item.message || "unknown_campaign_result")
      );
    }
  }
