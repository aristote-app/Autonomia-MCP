...
import { listRecentTerritorySignals } from "../db/territories.js";
...
async function loadMarketSignals() {
  if (!hasAutonomiaDatabase()) {
    return { opportunities: [], jobs: [], inboundLeads: [], territorySignals: [] };
  }

  try {
    const workspaceId = process.env.AUTONOMIA_DEFAULT_WORKSPACE_ID || null;
    const [opportunities, jobs, inboundLeads, territorySignals] = await Promise.all([
