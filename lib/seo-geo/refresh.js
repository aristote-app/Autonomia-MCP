...
async function loadMarketSignals() {
  if (!hasAutonomiaDatabase()) return { opportunities: [], jobs: [], inboundLeads: [] };

  try {
    const workspaceId = process.env.AUTONOMIA_DEFAULT_WORKSPACE_ID || null;
    const [opportunities, jobs, inboundLeads] = await Promise.all([
    ...
    return { opportunities: [], jobs: [], inboundLeads: [] };
  }
}
