...
function signalFromJob(item) {
  const signalKeys = Array.isArray(item?.signal_keys)
    ? item.signal_keys
    : [item?.signal_key].filter(Boolean);
  const text = [
    item?.title,
    item?.company_name,
    ...signalKeys,
    ...
    ...(Array.isArray(item?.use_cases) ? item.use_cases : []),
    item?.summary
  ].filter(Boolean).join(" ");
  const training = includesAny(text, TRAINING_TERMS);
  const territory = includesAny(text, TERRITORY_TERMS);
  return {
    query: item?.title || signalKeys[0] || "Besoin IA emploi",
    cluster: territory ? "Collectivités & territoires" : clusterFor(text),
    family: territory ? "territory-use-case" : training ? "training-use-case" : "execution-use-case",
    public_procurement_mentions: 0,
    territory_mentions: territory ? 1 : 0,
...
function signalFromTerritoryProgram(item) {
  const freshness = freshnessWeight(item?.occurred_at || item?.created_at);
  const source = String(item?.signal_payload?.source || "territory_program").toLowerCase();
  const text = [
    item?.title,
    item?.description,
    item?.signal_type,
    item?.signal_payload?.query,
    item?.signal_payload?.search_description
  ].filter(Boolean).join(" ");

  return {
    query: item?.title || "Programme territorial IA",
    cluster: "Collectivités & territoires",
    family: "territory-use-case",
    public_procurement_mentions: 0,
    territory_program_mentions: Math.max(0.25, freshness),
    territory_mentions: 0,
    inbound_mentions: 0,
    job_mentions: 0,
    revenue: 0,
    evidence_url: item?.source_url || null,
    source,
    source_text: text,
    detected_at: item?.occurred_at || item?.created_at || null,
    freshness_weight: freshness
  };
}
...
export function buildSeoGeoSignals({
  ...
  return [
    ...
    ...territoryPrograms.map(signalFromTerritoryProgram)
  ].filter((signal) => signal.query);
}

export function summarizeSeoGeoSignals(signals = []) {
  const territory = signals.filter(
    (signal) => signal.territory_mentions > 0 || signal.territory_program_mentions > 0
  ).length;
  const territoryPrograms = signals.filter(
    (signal) => signal.territory_program_mentions > 0
  ).length;
