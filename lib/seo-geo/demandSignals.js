...
function signalFromOpportunity(item, territoryDirectory = []) {
  ...
  const matchedTerritory = territoryDirectoryMatch(item?.buyer_name, territoryDirectory);
  const territory = includesAny(text, TERRITORY_TERMS) || Boolean(matchedTerritory);
