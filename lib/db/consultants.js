...
export async function listConsultantsWithSkills({
  ...
  let query = client
    .from("consultants")
    .select(
      "id,external_ref,display_name,status,available_from,tjm,currency,remote,locations,years_experience,notes,metadata,updated_at,consultant_skills(proficiency,years_experience,evidence,skills(id,canonical_name,category))"
    )
  ...
  return (data || []).map((consultant) => ({
    ...
    skills: (consultant.consultant_skills || [])
      .map((row) => row.skills?.canonical_name)
      .filter(Boolean),
    skill_details: (consultant.consultant_skills || []).map((row) => ({
      name: row.skills?.canonical_name || null,
