...
export async function getConsultantDiscoverySummary() {
  ...
  const discovered = rows.filter((row) => row.metadata?.source_platform);
  return {
    candidates: rows.filter((row) => row.status === "candidate").length,
    active: rows.filter((row) => row.status === "active").length,
    rejected: rows.filter((row) => row.status === "rejected").length,
    discovered_total: discovered.length,
    usable_discovered: discovered.filter((row) => row.status !== "rejected").length,
    malt: discovered.filter((row) => row.metadata?.source_platform === "malt").length,
    freelance_com: discovered.filter((row) => row.metadata?.source_platform === "freelance_com").length,
    linkedin: discovered.filter((row) => row.metadata?.source_platform === "linkedin").length,
    collective_work: discovered.filter((row) => row.metadata?.source_platform === "collective_work").length
  };
}
...
export async function upsertDiscoveredConsultantCandidates(
  ...
  for (const candidate of candidates || []) {
    ...
    const discoveryMetadata = {
      ...
      talent_family: candidate.talent_family || existingRow?.metadata?.talent_family || null,
      talent_cluster: candidate.talent_cluster || existingRow?.metadata?.talent_cluster || null,
      last_discovered_at: now,
      ...(actorUserId ? { last_discovered_by: actorUserId } : {})
    };

    const preservedStatus = ["active", "rejected"].includes(existingRow?.status)
      ? existingRow.status
      : "candidate";
    ...
    if (existingRow?.id) {
      ...
      if (existingRow.status === "rejected") rejectedPreserved += 1;
    } else {
      const result = await client
        .from("consultants")
        .insert({
  ...
  return {
    ...
    rejected_preserved: rejectedPreserved,
    skill_links: skillLinks,
    saved
  };
}
