...
export async function searchRankedOpportunities({
  ...
  let request = client
    ...
    .select(
      "id,opportunity_type,title,description,published_at,deadline_at,actionability_state,days_to_deadline,status,location,budget_min,budget_max,buyer_name,siren,ai_tags,is_ai_related,autonomia_fit_score,fit_coverage_percent,inferred_staffing_roles,first_seen_at,last_seen_at",
      { count: "exact" }
    )
