...
export async function ingestInboundLead(input) {
  ...
  const evidenceTasks = [persistEvidence(client, result.lead.id, input)];
  if (result.created || result.matchMethod !== "external_lead_id") {
    evidenceTasks.push(persistTouchpoint(client, result.lead.id, input));
  }
