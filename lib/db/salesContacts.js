...
export async function saveSalesContactCandidate({
  ...
  const payload = {
    ...
    metadata: {
      offer_track: commercialContext?.offer_track || null,
      trigger_source: commercialContext?.trigger_source || null,
      account_type: commercialContext?.account_type || null
    },
    relevance_score:
...
export async function setSalesContactOutreachStatus({
  ...
  const { error: eventError } = await client.from("sales_contact_events").insert({
    ...
    payload: {
      ...
      ...(updatePayload.next_action_at ? { next_action_at: updatePayload.next_action_at } : {})
    },
    created_by: actorUserId
  });
...
async function findUniqueExternalReplyContact({
  linkedinUrls = [],
  prospectIds = [],
  emails = []
} = {}) {
  ...
  if (prospectIds.length) {
    const { data, error } = await client
      .from("sales_contacts")
      .select("*")
      .in("waalaxy_prospect_id", prospectIds)
      .limit(5);
    if (error) throw error;
    add(data);
  }
  ...
  if (!candidates.size && emails.length) {
    const [b2b, direct] = await Promise.all([
      client.from("sales_contacts").select("*").in("email_b2b", emails).limit(5),
      client.from("sales_contacts").select("*").in("email_direct", emails).limit(5)
    ]);
    if (b2b.error) throw b2b.error;
  ...
  if (rows.length === 1) return { contact: rows[0], ambiguous: false };
  if (rows.length > 1) return { contact: null, ambiguous: true };
  return { contact: null, ambiguous: false };
}

export async function recordWaalaxyReply({
  identifiers,
  safeEvidence = {}
} = {}) {
  const match = await findUniqueExternalReplyContact(identifiers || {});
  ...
  const client = getAutonomiaServerClient();
  const now = new Date().toISOString();
  const terminal = ["won", "lost", "stopped"].includes(contact.outreach_status);

  if (!terminal && !contact.do_not_contact) {
  ...
  const { error: eventError } = await client
    ...
    .insert({
      ...
      payload: {
        ...
        status_changed: !terminal && !contact.do_not_contact,
  ...
  return {
    ...
    statusChanged: !terminal && !contact.do_not_contact
  };
}


export async function markSalesContactEnrichmentRequested({
  workspaceId,
  actorUserId,
  contactId
} = {}) {
  ...
  const { data: existing, error: existingError } = await client
    .from("sales_contacts")
    .select("id,verification_status,do_not_contact,enrichment_status")
    .eq("workspace_id", workspaceId)
    .eq("id", contactId)
    .maybeSingle();
  ...
  if (!existing) throw new Error("Contact not found");
  if (existing.verification_status !== "verified") {
    throw new Error("Verify the contact before enrichment");
  }
  if (existing.do_not_contact) {
    throw new Error("This contact is marked do-not-contact");
  }
...
export async function applySalesContactKasprResult({
  workspaceId,
  actorUserId,
  contactId,
  extracted,
  ...
  const client = getAutonomiaServerClient();
  const now = new Date().toISOString();
  const found = Boolean(extracted?.found);

  const update = {
    enrichment_provider: "kaspr",
    enrichment_status: found ? "enriched" : "not_found",
    enriched_at: found ? now : null,
    updated_by: actorUserId,
    updated_at: now
  };

  if (extracted?.email_b2b) update.email_b2b = extracted.email_b2b;
  if (extracted?.email_direct) update.email_direct = extracted.email_direct;
  ...
  const { error: eventError } = await client.from("sales_contact_events").insert({
    contact_id: contactId,
    workspace_id: workspaceId,
    event_type: found ? "enriched" : "enrichment_not_found",
    channel: "kaspr",
    payload: {
      provider: "kaspr",
      ...
      fields_found: {
        email_b2b: Boolean(extracted?.email_b2b),
        email_direct: Boolean(extracted?.email_direct),
...
export async function markSalesContactEnrichmentError({
  workspaceId,
  actorUserId,
  contactId,
  errorMessage
  ...
  // No provider payload is stored: only a short diagnostic message.
  await appendSalesContactEvent({
    workspaceId,
    actorUserId,
    contactId,
    eventType: "enrichment_not_found",
    channel: "kaspr",
    payload: {
      provider: "kaspr",
      failed: true,
      error: String(errorMessage || "Kaspr enrichment failed").slice(0, 300)
    }
  });
