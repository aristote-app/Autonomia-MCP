...
function normalizeJobSignalForUnifiedQueue(signal) {
  ...
  return {
    ...
    entity_id: signal.id,
    type_label: freelance ? "Mission freelance" : "Signal entreprise",
    source_label: signal.source_id || "source",
    ...
    triage_score: score,
    attention_bucket: freelance ? "Mission directe" : "Signal à exploiter",
    next_action: freelance
...
export function buildUnifiedTodayQueue({
  ...
  const normalizedSignals = (jobSignals || []).map(normalizeJobSignalForUnifiedQueue);
