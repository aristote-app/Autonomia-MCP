...
export function rankWaalaxyLists({ account = {}, lists = [] } = {}) {
  const track = inferOutreachTrack(account);
  const ranked = (lists || [])
    .map((list) => {
      const result = scoreCampaign(list?.name, track, account);
      const genericAutonomiaBonus = /autonomia|prospect|prospection|leads?/i.test(String(list?.name || "")) ? 1 : 0;
      return {
        ...list,
        autonomia_score: result.score + genericAutonomiaBonus,
        autonomia_track: track.id,
        autonomia_reason: result.matched.length
          ? "Correspondance : " + result.matched.join(", ")
          : genericAutonomiaBonus
            ? "Liste de prospection générique compatible"
            : "Aucune correspondance forte détectée"
      };
    })
    .sort((a, b) =>
  ...
  const recommended = ranked[0]?.autonomia_score >= 2 ? ranked[0] : null;
  return { track, recommended, ranked };
}
