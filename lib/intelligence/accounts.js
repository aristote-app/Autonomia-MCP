...
function dedupeEvents(events = []) {
  ...
  for (const event of events) {
    const url = text(event.source_url).toLowerCase();
    const title = text(event.title).toLowerCase().replace(/\s+/g, " ");
    const day = event.date ? String(event.date).slice(0, 10) : "";
    const key = url || [event.source_id || "", title, day].join("|");
...
function namedIntermediaryRisk(name = "") {
  const value = text(name);
  return INTERMEDIARY_PATTERNS.some((pattern) => pattern.test(value));
}

function concentrationRisk(events = []) {
  ...
  return maxSource / events.length >= 0.85;
}

function computeHeat(events = [], accountName = "") {
  ...
  const latestSignalAgeDays = events
    .map((event) => daysSince(event.date))
    .filter((age) => age != null)
    .sort((a, b) => a - b)[0] ?? null;
  ...
  const intermediaryRisk = concentrationRisk(events) || namedIntermediaryRisk(accountName);
  if (intermediaryRisk) score = Math.min(score, 58);

  // Historical volume must never keep an account artificially "hot".
  // Freshness caps are deliberately conservative and deterministic.
  if (latestSignalAgeDays == null) score = Math.min(score, 50);
  else if (latestSignalAgeDays > 120) score = Math.min(score, 35);
  else if (latestSignalAgeDays > 60) score = Math.min(score, 45);
  else if (latestSignalAgeDays > 30) score = Math.min(score, 60);

  const dormant = latestSignalAgeDays != null && latestSignalAgeDays > 60;
  const label = intermediaryRisk
    ? "À qualifier"
    : dormant
      ? "Dormant"
      : score >= 80
...
function whyNow(events, heat) {
  const reasons = [];
  if (heat.intermediaryRisk) {
    reasons.push("Volume élevé mais très concentré sur une seule source : vérifier s'il s'agit du client final ou d'un intermédiaire.");
  }
  if (heat.dormant) {
    reasons.push(`Dernier signal exploitable détecté il y a ${heat.latestSignalAgeDays} jours : ne pas prospecter comme un compte chaud sans nouvelle preuve.`);
  }
...
export function buildAccountIntelligence({ opportunities = [], jobSignals = [] } = {}) {
  ...
  return [...groups.values()]
    .map((account) => {
      const timeline = dedupeEvents(account.events).sort(
        (a, b) => new Date(b.date || 0).getTime() - new Date(a.date || 0).getTime()
      );
      const heat = computeHeat(timeline, account.name);
      ...
      const recommendedOffer = offers[0] || "Qualification IA";
      const primaryDecisionRole = decisionRoles(timeline)[0] || null;
      const accountType = heat.intermediaryRisk
        ? "intermediary"
        : timeline.some((event) => event.kind === "public")
          ? "public_buyer"
          : "end_client_candidate";
      ...
      return {
        ...
        intermediary_risk: heat.intermediaryRisk,
        signal_count: timeline.length,
        source_count: heat.sourceCount,
        recent_7d: heat.recent7,
        recent_30d: heat.recent30,
        latest_signal_age_days: heat.latestSignalAgeDays,
        dormant: heat.dormant,
        locations,
        sources,
        offers,
        recommended_offer: recommendedOffer,
        decision_roles: decisionRoles(timeline),
        primary_decision_role: primaryDecisionRole,
        why_now: whyNow(timeline, heat),
        next_action:
          heat.intermediaryRisk
            ? "Vérifier d'abord si ce compte est le client final ou un intermédiaire avant tout enrichissement."
            : heat.dormant
              ? "Attendre ou rechercher un nouveau signal avant toute prospection proactive."
              : heat.score >= 80
        ...
        playbook: {
          ...
          target_role: primaryDecisionRole?.label || "DSI / Direction digitale",
          trigger: timeline[0]?.title || null,
          proof_url: timeline[0]?.source_url || null
        },
        timeline
      };
    })
    .sort((a, b) => {
      ...
      return b.heat_score - a.heat_score || b.signal_count - a.signal_count;
    });
}
