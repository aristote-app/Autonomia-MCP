...
export function buildAccountOpportunityGraph(account = {}, { consultantMatches = [] } = {}) {
  const nodes = [];
  const edges = [];
  ...
  const resourceAnchor = account.recommended_offer
    ? safeId("offer", account.recommended_offer)
    : accountId;

  for (const match of (consultantMatches || []).slice(0, 5)) {
    const resourceId = safeId("resource", match.consultant_id || match.consultant_name);
    nodes.push(node(resourceId, "resource", match.consultant_name || "Consultant", {
      score: Number(match.score) || 0,
      matched_skills: match.matched_skills || [],
      available_from: match.available_from || null,
      tjm: match.tjm ?? null,
      currency: match.currency || "EUR",
      locations: match.locations || [],
      gaps: match.gaps || [],
      proof_url: match.proof_url || null,
      suitable_for_proactive_outreach: Boolean(match.suitable_for_proactive_outreach)
    }));
    edges.push(edge(resourceAnchor, resourceId, "peut être servi par"));
  }
  ...
  return {
    ...
    stats: {
      ...
      resources: nodes.filter((item) => item.type === "resource").length
    }
  };
}
