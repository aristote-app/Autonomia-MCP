...
export function validateGitHubDeploymentClaims(
  ...
  if (Number.isFinite(nbf) && nbf > nowSeconds + 30) return { ok: false, reason: "not_before" };
...
export async function verifyGitHubDeploymentToken(token) {
  ...
  if (parts.length !== 3) return { ok: false, reason: "malformed", claims: null };
  ...
  if (header.alg !== "RS256" || !header.kid) return { ok: false, reason: "header", claims: null };
